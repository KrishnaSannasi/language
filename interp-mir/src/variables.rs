...
impl<'a> Variables<'a> {
    pub fn layout(types: &'a [Type]) -> (Vec<usize>, Layout) {
        ...
        let size =
            count_bool * std::mem::size_of::<bool>() + count_i32 * std::mem::size_of::<i32>();

        let layout =
            Layout::from_size_align(size, align.max(std::mem::align_of::<bool>())).unwrap();
...
impl Get<bool> for Variables<'_> {
    fn get(&self, reg: usize) -> bool {
        ...
        unsafe { std::ptr::read(self.frame.ptr().add(index).cast()) }
    }
    ...
    fn get_mut(&mut self, reg: usize) -> &mut bool {
        ...
        unsafe { &mut *self.frame.ptr().add(index).cast() }
    }
}
