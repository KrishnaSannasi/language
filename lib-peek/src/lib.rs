...
pub type PeekableLexer<'str, 'idt, L, const N: usize> =
    Peekable<L, core_tokens::Token<'str, 'idt>, N>;
...
mod core_tokens_impl {
    ...
    impl<'str, 'idt, L: Lexer<'str, 'idt>, const N: usize> Lexer<'str, 'idt>
        for Peekable<L, Token<'str, 'idt>, N>
    {
        fn parse_token(&mut self) -> Option<Token<'str, 'idt>> {
            let inner = &mut self.inner;
            self.peek.try_pop_front().or_else(|| inner.parse_token())
        }
